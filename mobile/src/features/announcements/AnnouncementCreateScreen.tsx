...
import { createAnnouncement } from '../../api/endpoints/announcements';
...
type Nav = NativeStackNavigationProp<AppStackParamList, 'AnnouncementCreate'>;
...
export function AnnouncementCreateScreen() {
  ...
  const [targetRoleIds, setTargetRoleIds] = React.useState<number[]>([]);

  const rolesQuery = useQuery({ queryKey: ['roles', 'all'], queryFn: () => listRoles({ page: 1, limit: 100 }) });
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => createAnnouncement({ ...values, priority, targetRoleIds }),
    onSuccess: () => {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <View style={{ gap: 16 }}>
            ...
            <View>
              <Text style={[styles.label, { color: theme.textMuted }]}>Target roles (optional — leave empty for society-wide)</Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 6 }}>
                {(rolesQuery.data?.data ?? []).map((r) => {
                  const active = targetRoleIds.includes(r.id);
                  return (
                    <Pressable
                      key={r.id}
          ...
          <View style={{ marginTop: 20 }}>
            <Button label="Create announcement" onPress={handleSubmit((v) => mutation.mutate(v))} loading={mutation.isPending} />
          </View>
        </ScrollView>
