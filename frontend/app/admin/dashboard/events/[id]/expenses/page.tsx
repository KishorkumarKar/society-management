...
import { listEventExpenses, deleteEventExpense } from "@/lib/api/eventExpenses";
...
export default function EventExpensesPage() {
  ...
  const categories = useMemo(
    () => Array.from(new Set((rows ?? []).map((e) => e.category))).sort(),
    [rows]
  );
  ...
  async function handleDelete(target: BackendEventExpense) {
    try {
      await deleteEventExpense(target.id);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <p className="text-sm text-ink/50">
          ₹{totalSpent.toLocaleString("en-IN")} spent across {(rows ?? []).length} line item
          {(rows ?? []).length === 1 ? "" : "s"}.
        </p>
        {isManager && (
        ...
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody className="divide-y divide-ink/10">
            ...
            {rows !== null && filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="px-5 py-10 text-center text-ink/40">
                  {rows.length === 0 ? "No expenses recorded for this event yet." : "No expenses match your filters."}
                </td>
              </tr>
            )}
                  ...
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {isManager && (
                      <>
                        ...
                        <ConfirmDeleteButton label={expense.title} onConfirm={() => handleDelete(expense)} />
                      </>
                    )}
                  </div>
                </td>
